...
class RBFKernel:
    ...
    def __init__(self, params):
        ...
        self.n_features = self.n_params - 1
    ...
    def __call__(self, x, y, pairwise=True):
        ...
        k(x, y) = params[0] * \exp(-0.5 * \sigma_{i=1}^{i=n_features} params[i]* |x_{i} - y_{i}|^{2})
        :param x: (sample_size, n_features)
        :param y: (sample_size, n_features)
        ...
        exp_term = -0.5 * np.sum(self.params[1:] * (x - y) ** 2, axis=-1)
    ...
    def gradients(self, x, y, pairwise=True):
        ...
        grad_param_0 = np.exp(-0.5 * np.sum(self.params[1:] * (x - y) ** 2, axis=-1))
        # shape: (sample_size, sample_size) for pairwise
